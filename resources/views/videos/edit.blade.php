...
@section('content')
    <div class="flex justify-center">
        <div class="w-8/12 bg-white p-6 rounded-lg mb-4">
            <div>
                <h1 class="text-center mb-4 font-bold">Edit Video</h1>
                {!! Form::open(['url' => ['videos', $video->id], 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
                    <div class="mb-4">
                        {{Form::label('title', 'Title', ['class' => 'sr-only'])}}
                        {{Form::text('title', $video->title, ['class' => 'bg-gray-100 border-2 w-full p-3 rounded-lg', 'placeholder' => 'Title'])}}
                        @error('title')
